...
export default function DetailModal() {
  ...
  const handleReady = useCallback((player: Player) => {
    ...
    setMuted(!!player.muted()); // ensure boolean
  }, []);
    ...
    >
      <DialogContent sx={{ p: 0, bgcolor: "#181818" }}>
        <Box sx={{ position: "relative", mb: 3 }}>
          <Box sx={{ width: "100%", position: "relative", height: "calc(9 / 16 * 100%)" }}>
            <VideoJSPlayer
              options={{
                loop: true,
                autoplay: true,
                ...
                <Grid container spacing={5} alignItems="center">
                  <Grid item xs={12} sm={6} md={8}>
                    <Stack direction="row" spacing={1} alignItems="center">
                      ...
                      <Typography variant="body2">{detail.mediaDetail?.release_date?.substring(0, 4)}</Typography>
                      <AgeLimitChip label={`${getRandomNumber(20)}+`} />
                      <Typography variant="subtitle2">{formatMinuteToReadable(getRandomNumber(180))}</Typography>
            ...
            {/* Similar videos */}
            {similarVideos?.results && similarVideos.results.length > 0 && (
